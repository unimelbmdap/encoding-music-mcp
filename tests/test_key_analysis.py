...
def test_analyze_key_bach():
    ...
    assert isinstance(result["Confidence Factor"], (int, float)), "Confidence Factor should be numeric"
    assert 0.0 <= result["Confidence Factor"] <= 1.0, "Confidence Factor should be between 0 and 1"
...
def test_analyze_key_confidence():
    ...
    assert result["Confidence Factor"] > 0.5, "Well-defined tonal piece should have confidence > 0.5"
...
def test_analyze_key_format():
    ...
    assert "major" in key_name or "minor" in key_name, "Key name should specify major or minor"
